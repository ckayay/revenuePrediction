...
#########################################################################
# input paths
sales_and_pc_raw_d_loc = "../data/priceoptim/"
products_d_loc = "../data/priceoptim/products.csv"
stores_d_loc = "../data/priceoptim/stores.csv"
# output paths
df_sales_loc = "../data/priceoptim/aggregated_sales_data/"
# input path if exists, output path if not exist
df_stores_loc = "../data/priceoptim/stores_processed"
processed_time_d_loc = "../data/priceoptim/processed_time_df.csv"

#########################################################################
# 2: Data aggregation
# 2.1 process  products, stores, sales, price_changes data
...
# else: construct the group_val through clustering
    ...
    df_stores['AvgHouseholdIncome_std'] = (df_stores['AvgHouseholdIncome'] - df_stores['AvgHouseholdIncome'].mean()) / \
                                          df_stores['AvgHouseholdIncome'].std()
    df_stores['AvgTraffic_std'] = (df_stores['AvgTraffic'] - df_stores['AvgTraffic'].mean()) / df_stores[
        'AvgTraffic'].std()
...
sales_raw_file_name = "sales_store1_2019_01_02_00_00_00.json"
...
with open(sales_and_pc_raw_d_loc + sales_raw_file_name) as f:
   data = json.load(f)
...
df_sales = df_sales.rename(index=str, columns={"ProductID": "product_id", "StoreID": "store_id",
                                               "TransactionDateTime": "date_date"})
...
# 2.1 process price changes data
...
price_change_dates = "2019-01-02";

price_change_raw_file_name = sales_and_pc_raw_d_loc.append(sales_and_pc_raw_d_loc + 'pc_store' + str(i) + '_' + str(date) + '_00_00_00.json')

df_price_change = pd.DataFrame()

with open(price_change_raw_file_name) as f:
    data = json.load(f)
...
df_price_change = df_price_change.rename(index=str, columns={"ProductID": "product_id", "StoreID": "store_id",
                                                             "PriceDate": "week_start", "Price": "price"})
...
df_sales['week_start'] = df_sales['date_date'].apply(
    lambda x: df_sales_date_min + timedelta((x - df_sales_date_min).days / 7 * 7))
...
if (os.path.exists('data/priceoptim/df_sales.csv')):
